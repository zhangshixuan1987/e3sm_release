...
!-------------------------------------------------------------------------------
   ...
   real(r8) :: vp3_in(pverp)                    ! zonal wind third-order                       [m^3/s^3]
...
!   real(r8) :: ice_supersat_frac(pverp)
   ...
   real(r8) :: sclrp3(pverp,sclr_dim)           ! sclr'^3 (thermo. levels)                      [{units vary}^3]
...
!=======
   ...
   real(r8), pointer, dimension(:,:) :: vp2m_laststep, vp2t_laststep, vp2t_advtend !PMA
...
!PMA adds fields for gustiness
   ...
   do k=1,pver   ! loop over levels
     do i=1,ncol ! loop over columns
       ...
       um(i,k)   = state1%u(i,k)
       vm(i,k)   = state1%v(i,k)
       thlm(i,k) = ( state1%t(i,k) - (latvap/cpair)*state1%q(i,k,ixcldliq) ) &
                   * inv_exner_clubb(i,k)
   ...
   do k=1,pver
     do i=1,ncol
       thv(i,k) = state1%t(i,k) * inv_exner_clubb(i,k) &
                  * ( 1.0_r8 + zvir*state1%q(i,k,ixq) - state1%q(i,k,ixcldliq) )
     enddo
   enddo
...
!                     rtpthlp_in = rtpthlp_in + rtpthlp_mc_out * dtime
      ...
      do k=1,pverp
          ...
          up3(i,k)          = up3_in(pverp-k+1)
          vp3(i,k)          = vp3_in(pverp-k+1)
          ...
          rtp3(i,k)         = rtp3_in(pverp-k+1)
          thlp3(i,k)        = thlp3_in(pverp-k+1)
...
! Zhun need to check clubb_interp_advtend
               if (clubb_interp_advtend) then
...
!PMA adds gustiness and tpert
  ...
  result( delta_T_adj )
    ...
    ! Description:
    ! CLUBB conserves the vertical integral (including the effects of surface
